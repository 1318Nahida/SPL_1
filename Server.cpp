...
double verifyingSignature(int p,int q,double Signature,int privateKey){

    int n=func_n(p,q);
    double v=pow(Signature,(double)privateKey);
    v=fmod(v,n);
...
void decrypt(long int n)
{
    ...
    while((ch1=getc(filePointer))!=EOF){
        value+=ch1;
        //printf("The value is :%d",value);
    }

    value=(int)value%publicKey;
    printf("\nValue to be compared is : %lf \n",value);

    int privateKey=generatingPrivateKey(p,q,publicKey);
    double Signature=generatingSignature(p,q,value,publicKey);

    printf("Generated private key  : %d",privateKey);
    printf("\nThe signature is : %lf\n",Signature);

    double Verification=verifyingSignature(p,q,Signature,privateKey);

    printf("Decrypted value is :%lf \n",Verification);
